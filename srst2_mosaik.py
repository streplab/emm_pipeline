...
def parse_args():
	...
	parser.add_argument('--aligner', type=str, required=True, help = 'Type of aligner to be used, either bowtie2 or mosaik')
...
##############TARYN CHANGED srt(slope) to srt(test)############################################### 
...
def run_mosaik(mapping_files_pre, sample_name, fastqs, args, db_name, db_full_path, gene_names,db_dat_out,db_15,fastq_dat):
	
	#db_dat_out = db_name + ".dat"	

	#db_dat = ['MosaikBuild', '-fr', db_full_path, '-oa', db_dat_out]

	#logging.info('MosaikBuild of' + db_dat_out)

	#run_command(db_dat)

	#db_15 = db_name + "_15"

	#db_jump = ['MosaikJump', '-ia', db_dat_out, '-out', db_15, '-hs', '15']

	#logging.info('MosaikJump of' + db_15)

	#run_command(db_jump)
	
	fastq_dat = ['MosaikBuild']

	if len(fastqs)==1:
		fastq_dat += ['-q', fastqs[0]]
	elif len(fastqs)==2:
		fastq_dat += ['-q', fastqs[0], '-q2', fastqs[1]]

	sample_dat = sample_name + ".dat"

	fastq_dat += ['-out', sample_dat, '-st', 'illumina']

	logging.info('MosaikBuild of' + sample_dat)

	run_command(fastq_dat)

	#unaligned_fastq = sample_name + "-unaligned.fastq"

	sample_db_dat = sample_name + "-" + db_name + ".dat"

	aligner = ['MosaikAligner', '-in', sample_dat, '-ia', db_dat_out, '-hs', '15', '-mm', '12', '-act', '35', '-mhp', '100', '-m', 'all', '-p', '24', '-bw', '29', '-j', db_15, '-out', sample_db_dat]

	logging.info('Aligning' + sample_name)

	run_command(aligner)

	sample_db = sample_name + "-" + db_name
	
	coverage = ['MosaikCoverage', '-in', sample_db_dat, '-ia', db_dat_out, '-od', './', '-cg']

	logging.info('Running MosaikCoverage for' + sample_name)

	run_command(coverage)

	sample_db_unsorted_bam = sample_name + "-" + db_name + "_unsorted.bam"

	text = ['MosaikText', '-in', sample_db_dat, '-bam', sample_db_unsorted_bam]

	logging.info('Creating bam file for' + sample_name)

	run_command(text)

	bam_sort = ['samtools', 'sort', sample_db_unsorted_bam, sample_db]

	logging.info('Sorting bam file for' + sample_name)

	run_command(bam_sort)

	bam = sample_name + "-" + db_name + ".bam"

	sam = sample_name + "-" + db_name + ".sam"

	create_sam = ['samtools', 'view', '-h', '-o', sam, bam]

	logging.info('creating sam file for' + sample_name)

	run_command(create_sam)

	for fl in glob.glob("*.cov"):
		os.remove(fl)

	for gene in gene_names:
		pdfs = gene + "*.pdf"
		for fl in glob.glob(pdfs):
			newname = sample_name + "-" + fl		
			os.rename(fl, newname)

	for fl in glob.glob("*.ps"):
		os.remove(fl)

	#for fl in glob.glob("*.jmp"):
	#	os.remove(fl)

	#for fl in glob.glob("*.dat"):
	#	os.remove(fl)

	os.remove(sample_db_unsorted_bam)
...
def get_pileup(mapping_files_pre,args,raw_bowtie_sam,bowtie_sam_mod,fasta,pileup_file):
	...
	logging.info('Processing Alignment output with SAMtools...')
	...
	with open(pileup_file, 'w') as sam_pileup:
		run_command(['samtools', 'mpileup', '-L', '1000', '-f', fasta,
					 '-Q', str(args.baseq), '-q', str(args.mapq), '-A', out_file_bam_sorted + '.bam'],
					 stdout=sam_pileup)
...
def process_fasta_db(args, fileSets, run_type, db_reports, db_results_list, fasta):
	...
	if(args.aligner == "mosaik"):
		db_dat_out = db_name + ".dat"	
		db_dat = ['MosaikBuild', '-fr', fasta, '-oa', db_dat_out]
		logging.info('MosaikBuild of' + db_dat_out)
		run_command(db_dat)
		db_15 = db_name + "_15"
		db_jump = ['MosaikJump', '-ia', db_dat_out, '-out', db_15, '-hs', '15']
	...
	for sample_name in fileSets:
		...
		try:
			...
			gene_list, results = \
				map_fileSet_to_db(args,sample_name,fastq_inputs,db_name,fasta,size,gene_names,db_dat_out,db_15,fastq_dat,\
...
def map_fileSet_to_db(args,sample_name,fastq_inputs,db_name,fasta,size,gene_names,db_dat_out,db_15,fastq_dat,\
	unique_gene_symbols, unique_allele_symbols,run_type,ST_db,results,gene_list,db_report):
	...
	if args.use_existing_scores and os.path.exists(scores_file):
		...
		if args.use_existing_pileup and os.path.exists(pileup_file):
			...
			if args.aligner=='bowtie2':			
				...
				bowtie_sam = run_mosaik(mapping_files_pre,sample_name,fastq_inputs,args,db_name,fasta, gene_names,db_dat_out,db_15,fastq_dat)
			else:
				print(args.aligner + 'is not an acceptable aligner type')
...
def main():
	...
	if args.aligner != 'bowtie2' and args.aligner != 'mosaik':
		print('Aligner must be of type bowtie2 or mosaik')
		logging.info('Aligner not found')
		sys.exit()
